...
__global__ void transpose_Matrix(int* Matrix, int* Matrix_transpose, int q, int r)
{
    ...
    if (row < q && colm < r)
    {
        int index_in = row * r + colm;
        int index_out = colm * q + row;
        
        Matrix_transpose[index_out] = Matrix[index_in];
    }
}

//Function to compute multiplication of CD^T and AB:In this 
__global__ void matrix_Multiply(int *A,int *B,int *AB,int p, int q, int r) {
...
*/
   ...
   sc[threadIdx.x] = A[row_c*q + threadIdx.x] * B[row_d * q + threadIdx.x] ;
   __syncthreads();
...
void computE(int p, int q, int r, int *h_matrixA, int *h_matrixB, int *h_matrixC, int *h_matrixD, int *h_matrixE){
	...
	/* Configure and launch kernels */
      ...
      matrix_Multiply<<<blocksize,q>>>(d_matrixC,d_matrixD,CD_device,p,q,r);
      ...
      matrix_Multiply<<<blocksize,q>>>(d_matrixA,b_transpose_device,AB_device,p,q,r);
