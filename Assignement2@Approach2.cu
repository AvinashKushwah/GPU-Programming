...
__global__ void matrix_MultiplyAB(int *A, int *B, int *AB, int p, int q, int r,int BLOCK_SIZE) {
...
void computE(int p, int q, int r, int *h_matrixA, int *h_matrixB, int *h_matrixC, int *h_matrixD, int *h_matrixE){
    ...
    int BLOCK_SIZE=32;
      ...
      matrix_MultiplyAB<<<GridDim_forAB, blockDimfor_AB>>>(d_matrixA,d_matrixB,AB_device,p,q,r,BLOCK_SIZE);
      ...
      dim3 numBlocks((r + 15) / 16, (p + 15) / 16);
